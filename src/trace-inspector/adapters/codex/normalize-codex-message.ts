...
export function normalizeCodexMessage(
  ...
  switch (message.method) {
        ...
        return [
          makeEvent(
            ...
            `${isAgent ? "Agent" : "User"} message started`,
            {
              entityId: readString(item, "id"),
              status: "running",
              // Starting a streamed message is runtime lifecycle evidence. The
              // model-reported content is emitted once, from item/completed.
    ...
    case "item/agentMessage/delta":
      // Stream deltas remain in raw.jsonl for byte-level auditability. Omitting
      // them here prevents every token/chunk from becoming a timeline event;
      // item/completed below supplies the complete message as one event.
      return [];
      ...
      if (
        ...
        return [
          makeEvent(
            ...
            "message.completed",
            isAgent
              ? `Agent message: ${text.length > 0 ? text : "(empty)"}`
              : "User message completed",
            {
              ...
              attributes: isAgent
                ? {
                    ...params,
                    messageText: text,
                    messagePhase: readString(item, "phase"),
                  }
                : params,
            },
          ),
