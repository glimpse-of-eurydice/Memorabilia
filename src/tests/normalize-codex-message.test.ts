...
test("collapses streamed agent output into one complete model-reported event", () => {
  const delta: RawTraceRecord = {
    receivedAt: "2026-07-30T19:00:00.000Z",
    sequence: 3,
    payload: {
  ...
  const completed: RawTraceRecord = {
    receivedAt: "2026-07-30T19:00:01.000Z",
    sequence: 4,
    payload: {
  ...
  assert.deepEqual(normalizeCodexMessage(delta, context), []);
  const events = normalizeCodexMessage(completed, context);
  const [event] = events;

  assert.equal(events.length, 1);
  assert.equal(event?.kind, "message.completed");
  assert.equal(event?.evidenceLevel, "model_reported");
  assert.equal(event?.title, "Agent message: Hello world");
  assert.equal(event?.attributes.messageText, "Hello world");
  assert.equal(event?.rawRef.sequence, 4);
});

test("treats agent message start as observed runtime lifecycle", () => {
  const [event] = normalizeCodexMessage(
    {
      receivedAt: "2026-07-30T19:00:00.000Z",
      sequence: 2,
      payload: {
        method: "item/started",
        params: { item: { type: "agentMessage", id: "message_001" } },
      },
    },
    context,
  );

  assert.equal(event?.kind, "message.started");
  assert.equal(event?.evidenceLevel, "observed");
});
